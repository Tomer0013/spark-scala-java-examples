...
public class MaxTemperatures {
    ...
    public static void main(String[] args) {
        ...
        JavaRDD<Tuple3<String, String, Float>> maxTemps = parsedLines.filter(x -> x._2().equals("TMAX"));
        JavaPairRDD<String, Float> stationTemps = maxTemps.mapToPair(x -> new Tuple2<>(x._1(), x._3()));
        JavaPairRDD<String, Float> maxTempsByStation = stationTemps.reduceByKey(Math::max);
        List<Tuple2<String, Float>> results = maxTempsByStation.collect();
        results.stream().sorted(Comparator.comparing(Tuple2::_1)).collect(Collectors.toList()).forEach(
                x -> System.out.println(x._1() + " maximum temperature: " + String.format("%.2f", x._2()) + " F")
        );
    }
}
